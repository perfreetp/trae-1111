...
const InventoryPage: React.FC = () => {
  ...
  const levelMap: Record<string, { color: string; text: string }> = {
    ...
    critical: { color: 'red', text: '过效期' },
  };

  const handleGenerateProcess = () => {
    const pendingItems = nearExpiryItems.filter((item) => {
      const existing = processList.find(
        (p) => p.id === item.id && p.status === 'pending'
      );
      return !existing;
    });

    if (pendingItems.length === 0) {
      message.warning('没有需要处理的近效期药品，或所有药品已在处理清单中');
      return;
    }
  ...
  const handleExportProcessList = () => {
    ...
    const rows = processList.map((item) => [
      item.drugName,
      item.batchNo,
      item.expiryDate,
    ...
    <div className="space-y-6">
          ...
          <Card className="border-0 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-500">{nearExpiryItems.length}</p>
              </div>
      ...
      >
        <div className="mb-4 flex gap-3">
          <Select defaultValue="all" style={{ width: 140 }}>
            ...
            <Option value="critical">过效期</Option>
          </Select>
      ...
      >
        <div className="space-y-4">
          <Alert
            message="即将生成近效期处理清单"
            description={`系统将自动筛选出 ${nearExpiryItems.length} 种近效期药品，生成处理清单供您确认。`}
            type="warning"
            showIcon
          />
          <List
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <div>
                ...
                <Tag color="green">已处理: {processList.length - pendingCount}</Tag>
              </div>
            </div>
            <Table
              rowKey="id"
        ...
        {selectedItem && (
          <div className="space-y-4">
            ...
            {selectedItem.processRecords && selectedItem.processRecords.length > 0 && (
              <div>
                <p className="font-medium mb-2">处理记录</p>
                <List
                  size="small"
                  bordered
                    ...
                    <List.Item>
                      <div className="flex justify-between w-full">
                        <div>
                          ...
                          <span className="mx-2">
                            {record.type === 'return_supplier' ? '退回供应商' :
                             record.type === 'destroy' ? '销毁处理' :
                             record.type === 'discount_sale' ? '折价销售' : '其他'}
                          </span>
                          <span className="text-blue-500">-{record.quantity}</span>
                        </div>
                        <span className="text-gray-400 text-sm">{record.notes || '-'}</span>
                      </div>
                    </List.Item>
                  )}
                />
